...
object UserSalesAnalysisController extends Serializable with UtilsFunctions {
  ...
  def main(args: Array[String]): Unit = {
    val Array(configFilePath, salesDataPath, apiUserURL) = args
    ...
    userLocCordDF.collect().foreach { row =>
      ...
      val apiWeatherUrl = "https://api.openweathermap.org/data/2.5/weather?lat=" + lat + "&lon=" + lng + "&appid=0f5d13c7e5474ad7b0e16c44dae65c39"
      // Read API response and append to lines DataFrame

      weatherDF = spark.read.json(spark.sparkContext.parallelize(Seq(Source.fromURL(apiWeatherUrl).getLines().mkString("\n"))))
